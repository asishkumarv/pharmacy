                  ...
                  <TableBody>
                    {displayedData.length > 0 ? displayedData.map((item, index) => (
                      <TableRow key={index} hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                        <TableCell sx={{ fontWeight: 500, color: 'var(--primary)' }}>{item.lcCode || '-'}</TableCell>
                        <TableCell>{item.lcName || '-'}</TableCell>
                        <TableCell>{item.city || '-'}</TableCell>
                        <TableCell>{item.parentName || '-'}</TableCell>
                        <TableCell>{item.addedDate ? new Date(item.addedDate).toLocaleDateString() : '-'}</TableCell>
                      </TableRow>
                    )) : (
                      <TableRow>
                        <TableCell colSpan={5} align="center" sx={{ py: 3, color: 'text.secondary' }}>No customers found</TableCell>
                      </TableRow>
                    )}
                  </TableBody>
                </Table>
